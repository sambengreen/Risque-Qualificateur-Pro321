...
export default function TutoprevPedagogie() {
  ...
  const tutoprevResources = useMemo(() => {
    return allResources.filter(r => r.category === "TutoPrev' Pedagogie");
  }, [allResources]);
  ...
  return (
    <div className="p-6 space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Collection INRS de formation a la maitrise des risques professionnels par secteur d'activite
            </p>
          </div>
        </div>
        <div className="mt-2 p-4 rounded-lg border bg-blue-50 dark:bg-blue-950/30 border-blue-200 dark:border-blue-800">
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm text-muted-foreground">
              <strong className="text-foreground">TutoPrev'</strong> est une demarche de formation basee sur l'observation
              et l'analyse de situations reelles de travail en entreprise. Destinee aux lycees professionnels, CFA et
              entreprises d'accueil, elle favorise l'integration de la prevention des risques dans les enseignements
              en rapprochant l'ecole et l'entreprise.
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1 max-w-md">
          ...
          <Input
            ...
            className="pl-9"
            data-testid="input-search-tutoprev"
          />
          {searchTerm && (
            <Button
              variant="ghost"
        ...
        {sectors.map(sector => {
          ...
          return (
            <Badge
              ...
              data-testid={`filter-sector-${sector.replace(/\s+/g, '-').toLowerCase()}`}
            >
          ...
          {filteredResources.map(resource => {
            ...
            return (
              <Card
                ...
                data-testid={`card-tutoprev-${resource.edNumber.replace(/\s+/g, '-').toLowerCase()}`}
              >
                <CardContent className="p-5 flex flex-col gap-3 flex-1">
                  ...
                  <div className="flex items-center gap-2 mt-auto pt-2">
                    <Button
                      ...
                      data-testid={`button-toggle-${resource.edNumber.replace(/\s+/g, '-').toLowerCase()}`}
                    >
                      {isExpanded ? (
                        <>
                    ...
                    {resource.inrsUrl && (
                      <Button
                        ...
                        className="text-xs"
                        asChild
                        data-testid={`button-pdf-${resource.edNumber.replace(/\s+/g, '-').toLowerCase()}`}
                      >
      ...
      <div className="text-center text-xs text-muted-foreground pt-4 border-t">
        <p>
          Collection TutoPrev' Pedagogie — Source : INRS (Institut national de recherche et de securite)
        </p>
        <p className="mt-1">
